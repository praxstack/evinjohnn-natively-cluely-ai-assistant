...
export interface RegistryEntry {
  ...
  /** Human name, when the registry supplies one. Falls back to the id. */
  name?: string;
  /**
   * Published artefacts, present only on a registry generated by a release.
   * Absent means the entry is discoverable but not installable in-app — the
   * user clones and installs from a folder instead.
   */
  download?: {
    code: string;
    manifest: string;
  ...
  /**
   * External binaries the extension spawns and does NOT bundle (llama-server
   * for the GGUF adapters). Surfaced so the UI can say a runtime is required
   * rather than letting an install look ready when it is absent.
   */
  requiresExternalRuntime?: string[];
}

/** Narrow an untrusted registry row's download block, or drop it entirely. */
function readDownload(raw: any): RegistryEntry['download'] | undefined {
  const d = raw?.download;
  if (!d || typeof d !== 'object') return undefined;
  const code = typeof d.code === 'string' ? d.code : '';
  const manifest = typeof d.manifest === 'string' ? d.manifest : '';
  const shaCode = typeof d.sha256?.code === 'string' ? d.sha256.code : '';
  const shaManifest = typeof d.sha256?.manifest === 'string' ? d.sha256.manifest : '';
  // A half-formed download block is worse than none: it would render an
  // Install button that cannot succeed. Require all four fields.
  if (!code || !manifest || !/^[a-f0-9]{64}$/i.test(shaCode) || !/^[a-f0-9]{64}$/i.test(shaManifest)) {
    return undefined;
  }
  const bytes = d.bytes && typeof d.bytes === 'object'
    ? {
        code: Number(d.bytes.code) || 0,
        manifest: Number(d.bytes.manifest) || 0,
      }
    : undefined;
  return { code, manifest, sha256: { code: shaCode, manifest: shaManifest }, ...(bytes ? { bytes } : {}) };
}
  ...
  try {
    ...
    const entries = rows
      ...
      .map((r) => ({
        id: String(r.id),
        repo: String(r.repo),
        // A release registry names the field `version`; the older metadata-only
        // shape used `latestVersion`. Accept both rather than showing a blank.
        latestVersion: String(r.latestVersion ?? r.version ?? ''),
        apiVersion: String(r.apiVersion ?? ''),
        category: String(r.category ?? ''),
        modelLicenses: Array.isArray(r.modelLicenses) ? r.modelLicenses.map(String) : undefined,
        // Spread rather than assign-undefined: a key present with an undefined
        // value still shows up in Object.keys, so a metadata-only registry
        // would appear to carry a `download` field it does not have.
        ...(typeof r.name === 'string' && r.name ? { name: r.name } : {}),
        ...(readDownload(r) ? { download: readDownload(r)! } : {}),
        ...(Array.isArray(r.requiresExternalRuntime)
          ? { requiresExternalRuntime: r.requiresExternalRuntime.map(String) }
          : {}),
      }));
