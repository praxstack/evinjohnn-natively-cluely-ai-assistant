    ...
    - **Google STT: selecting Chinese no longer kills transcription for the whole meeting** (PR #494 by @averatec0773, with a follow-up that closes the wider class): Picking **Google** as the STT provider and **Chinese** as the language produced zero captions for the entire session, silently, while English worked. Two independent mismatches lined up only for Chinese. `RECOGNITION_LANGUAGES` resolves Chinese to `zh-CN` and `GoogleSTT` forwarded that verbatim as Google's `languageCode`, but STT v1 has no `zh-CN` — Mandarin is `cmn-Hans-CN` (Traditional is `cmn-Hant-TW`); and `startStream()` hardcoded `model: 'latest_long'`, which v1 does not offer for Mandarin at all. Either one is rejected on the very first `streamingRecognize` with gRPC `INVALID_ARGUMENT` (code 3), which is in `PERMANENT_GRPC_CODES` → `isFatalError = true` → the guard at the top of `write()` drops every subsequent audio chunk with no retry. #494 translates `zh-*` → `cmn-*` inside the Google adapter (the shared map still exposes `zh-CN`, because Deepgram/Soniox want it) and falls back to the `default` model for the two Mandarin codes; every other language keeps `latest_long` byte-for-byte.

      The follow-up fixes the *class* rather than the instance. A static two-entry allowlist cannot be proven complete: Google retired the STT **v1** language × model table (`/speech-to-text/docs/languages` and `/speech-to-text/docs/speech-to-text-supported-languages` both now redirect to the v2 doc, which uses `chirp`/`long`/`short` and has no `latest_long` column), so any *other* locale lacking `latest_long` was still a silent total failure waiting to happen. `GoogleSTT` now treats the **first** code-3 rejection of a `latest_long` stream as a model problem rather than a permanent one: it downgrades to `model: 'default'`, lets `write()`'s existing lazy reconnect reopen the stream, and does **not** re-emit the error (main.ts's consecutive-error counter would tear the session down anyway — the same reasoning that already keeps the code-11 idle timeout quiet). The retry is bounded: a second code 3, or one raised while already on `default`, is permanent exactly as before, and codes 7/16 (`PERMISSION_DENIED`/`UNAUTHENTICATED`) are untouched since no model change can fix auth. The model choice moved into one `resolveModel()` helper so the static allowlist and the runtime downgrade apply at a single point, and a downgrade is cleared on `start()` and on a language change — it is a fact about one stream attempt, not a permanent property of the instance. A locale that is genuinely unsupported now costs one failed round-trip at the start of a meeting instead of the whole meeting's captions. New `electron/audio/__tests__/GoogleSttInvalidArgumentModelDowngrade.test.mjs` drives the real compiled `GoogleSTT` against a recording stub `SpeechClient` and asserts the model that actually goes on the wire across all six paths (supported → `latest_long`; known-unsupported → `default` with no round-trip; code 3 → downgrade, no fatal, no emit; second code 3 → fatal *and* emitted; 7/16 → immediately fatal; fresh `start()` → downgrade cleared); the two downgrade cases were verified to fail against the pre-follow-up build. #494's own `GoogleSttMandarinLatestLongFallback.test.mjs` was made location-agnostic in the same commit, since hoisting the decision into `resolveModel()` broke its assertion that the guard lived inside `startStream()`'s body while the contract itself still held.

    - **Chinese meeting PDFs render instead of coming out as boxes** (PR #501 by @averatec0773, with review follow-ups): jsPDF's built-in Helvetica is WinAnsi/Latin-only with no CIDFont, so every CJK codepoint in an exported meeting was an empty box or was dropped outright, and long Chinese lines overflowed the page because `splitTextToSize` breaks on spaces and CJK has none. When a meeting contains CJK the exporter now embeds a GB2312 + Latin subset of **Noto Sans SC** (SIL OFL 1.1, `src/assets/fonts/LICENSE-NotoSansSC.txt`) and the output carries CIDFont + Identity-H + ToUnicode, so Chinese renders correctly and stays selectable and copyable; wrapping switches to per-character break units for CJK while keeping Latin words whole. The font is pulled as a lazy ~2.8 MB `import()` code-split chunk rather than `fetch()`ed, because the packaged renderer runs on `file://` where Chromium's Fetch API refuses local URLs — so English-only exports and app startup never load it. Two review follow-ups shipped with the merge: (1) the font is applied **per run** rather than document-wide, because the subset carries only 1 of the 96 Latin-1 Supplement codepoints and jsPDF's Identity-H encoder does not merely drop an unmappable `U+0080–00FF` character — it discards the rest of the string, so "José Müller said the room was 25°C" was silently exported as "Jos"; each run now keeps the font that can encode it (the two encoders split exactly at `U+00FF`), which also recovers `—` and `…`; (2) the CJK detection regex and the line-break regex spelled the same four ranges twice with literal characters and silently disagreed — one used 豈 `U+8C48`, the other 豈 `U+F900` — and with no `u` flag the wider range covered `U+D800–DFFF`, so every surrogate half matched on its own and split emoji into two break units while breaking Hangul per character; both are now built from one escaped source string with `u`. New `src/lib/pdfCjkText.mjs` holds the pure helpers with 17 tests under `npm run test:lib`; the two homoglyph guards were verified to fail against the original regexes. Still dropped (pre-existing, unchanged in kind): `•` and `“ ”`, which neither font can encode.

    - **Windows: the shortcut guard is now on by default, with a Settings toggle (`Protect Natively shortcuts`)**: The guard is the always-on `WH_KEYBOARD_LL` hook that swallows and self-dispatches Natively's own global chords, so a `RegisterHotKey` registration the OS silently dropped (sleep/wake, display change, workspace switch) can no longer leak its completing character into whatever you were typing in — a stray newline from `Ctrl+Enter`, a digit from `Ctrl+1`. It was previously opt-in and off, which meant nobody got the fix. It now defaults on, and **Settings → Protect Natively shortcuts** is the opt-out for anyone whose antivirus/EDR objects to a persistent low-level keyboard hook (the original reason for defaulting off). The guard's chord table is now filtered by the active launcher/overlay mode and re-armed on every mode switch, so the hook can no longer dispatch a shortcut that is disabled in the current mode, and `Ctrl+↑/↓` plus `Ctrl+Alt+←/→` (the scroll binds) are now covered — they were previously dropped from the table and left to `RegisterHotKey` alone. Windows-only; macOS uses `CGEventTap` and has no `RegisterHotKey` drop to guard against, so nothing changes there and the toggle is not shown. Swallowing is still strictly limited to chords the app has actually registered: bare modifiers are never swallowed, `Win` combos return to the OS untouched, and `Ctrl+Alt` is accepted only with an arrow key so AltGr-generated text (`@ € { } \ ~` on EU layouts) can never be intercepted.
